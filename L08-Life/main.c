...
register uint8 row;
register uint8 col;
register uint16 position;
register uint16 top_row;
register uint16 middle_row;
register uint16 bottom_row;
uint16 new_row;
uint8 left_three;
uint8 middle_three;
uint8 right_three;
...
void main(void)
{
	...
	while (1)							// new pattern seed
	{
		...
		while (1) {						// next generation
			...
			row = 78;
			while (row > 0) {
				top_row = life[row + 1][4];
				middle_row = life[row][4];
				...
				while (col > 0) {
					...
					neighbors += bit_value;
					bit_value = (middle_row & position) ? 0 : 1;
					neighbors -= bit_value;
					...
					if (position) position >>= 1;
						...
						top_row = life[row][(col >> 4) - 1];
						middle_row = life[row][(col >> 4) - 1];
						bottom_row = life[row][(col >> 4) - 1];
						position = 0x8000;
					}
					left_three = 0;
					left_three += (top_row & position) ? 1 : 0;
					left_three += (middle_row & position) ? 1 : 0;
